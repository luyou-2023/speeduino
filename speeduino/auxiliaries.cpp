...
{
  bool nitrousOn = false; // 变量用来跟踪控制是否在任何时刻被激活。

  if(configPage10.n2o_enable > 0) // 检查氮气控制是否启用
  {
    bool isArmed = READ_N2O_ARM_PIN(); // 读取氮气启动引脚的状态
    if (configPage10.n2o_pin_polarity == 1) {
        isArmed = !isArmed; // 如果氮气在引脚低电平时激活，则翻转读取值（n2o_pin_polarity = 0 时，高电平激活）
    }

    // 执行主要检查以确定氮气是否准备好
    if( (isArmed == true) &&
        (currentStatus.coolant > (configPage10.n2o_minCLT - CALIBRATION_TEMPERATURE_OFFSET)) && // 冷却液温度大于最小温度阈值
        (currentStatus.TPS > configPage10.n2o_minTPS) && // 节气门位置传感器 (TPS) 大于最小阈值
        (currentStatus.O2 < configPage10.n2o_maxAFR) && // 氧气传感器 (O2) 小于最大空气燃料比 (AFR)
        (currentStatus.MAP < (uint16_t)(configPage10.n2o_maxMAP * 2)) ) // 绝对进气压力 (MAP) 小于最大压力的两倍
    {
      // 配置页面中的值被除以100以适应字节范围。将它们乘回实际值。
      uint16_t realStage1MinRPM = (uint16_t)configPage10.n2o_stage1_minRPM * 100; // 阶段1最小RPM
      uint16_t realStage1MaxRPM = (uint16_t)configPage10.n2o_stage1_maxRPM * 100; // 阶段1最大RPM
      uint16_t realStage2MinRPM = (uint16_t)configPage10.n2o_stage2_minRPM * 100; // 阶段2最小RPM
      uint16_t realStage2MaxRPM = (uint16_t)configPage10.n2o_stage2_maxRPM * 100; // 阶段2最大RPM

      // 氮气状态初始设置为 0，然后根据不同阶段添加状态
      // OFF    = 0
      // STAGE1 = 1
      // STAGE2 = 2
      // BOTH   = 3 (即 STAGE1 + STAGE2 = BOTH)
      currentStatus.nitrous_status = NITROUS_OFF; // 重置当前状态
  ...
  if (nitrousOn == false) // 如果氮气控制未激活
  {
    currentStatus.nitrous_status = NITROUS_OFF;
    BIT_CLEAR(currentStatus.status3, BIT_STATUS3_NITROUS);

    if(configPage10.n2o_enable > 0) // 如果氮气控制已启用
    {
      N2O_STAGE1_PIN_LOW(); // 禁用阶段1氮气引脚
      N2O_STAGE2_PIN_LOW(); // 禁用阶段2氮气引脚
    }
  }
}
