...
// === 16x16 三维表格（按 RPM 与负载轴）===
struct table3d16RpmLoad fuelTable;              ///< 主喷油表（16x16）
struct table3d16RpmLoad fuelTable2;             ///< 副喷油表（16x16）
struct table3d16RpmLoad ignitionTable;          ///< 主点火提前角表（16x16）
struct table3d16RpmLoad ignitionTable2;         ///< 副点火提前角表（16x16）
struct table3d16RpmLoad afrTable;               ///< 目标空燃比表（16x16）

// === 8x8 三维表格 ===
struct table3d8RpmLoad stagingTable;            ///< 阶段喷油表（8x8）
struct table3d8RpmLoad boostTable;              ///< 增压目标表（8x8）
struct table3d8RpmLoad boostTableLookupDuty;    ///< 增压查找占空比表（8x8）
struct table3d8RpmLoad vvtTable;                ///< 可变气门正时控制表1（8x8）
struct table3d8RpmLoad vvt2Table;               ///< 可变气门正时控制表2（8x8）
struct table3d8RpmLoad wmiTable;                ///< 水甲醇喷射控制表（8x8）

// === 6x6 燃油微调表 ===
trimTable3d trim1Table;                         ///< 燃油微调表1（6x6）
trimTable3d trim2Table;                         ///< 燃油微调表2（6x6）
trimTable3d trim3Table;                         ///< 燃油微调表3（6x6）
trimTable3d trim4Table;                         ///< 燃油微调表4（6x6）
trimTable3d trim5Table;                         ///< 燃油微调表5（6x6）
trimTable3d trim6Table;                         ///< 燃油微调表6（6x6）
trimTable3d trim7Table;                         ///< 燃油微调表7（6x6）
trimTable3d trim8Table;                         ///< 燃油微调表8（6x6）

// === 4x4 三维表格 ===
struct table3d4RpmLoad dwellTable;              ///< 点火维持时间表（Dwell，4x4）

// === 2D 单轴表格 ===
struct table2D taeTable;                        ///< TPS 加速富油修正表（4点）
struct table2D maeTable;                        ///< MAP 加速富油修正表
struct table2D WUETable;                        ///< 冷启动富油修正表（10点）
struct table2D ASETable;                        ///< 启动后富油补偿表（4点）
struct table2D ASECountTable;                   ///< 启动后富油持续时间表（4点）
struct table2D PrimingPulseTable;               ///< 点火前预喷油脉宽表（4点）
struct table2D crankingEnrichTable;             ///< 启动喷油富集表（4点）
struct table2D dwellVCorrectionTable;           ///< 点火维持时间电压修正表（6点）
struct table2D injectorVCorrectionTable;        ///< 喷油器电压修正表（6点）
struct table2D injectorAngleTable;              ///< 喷油器角度控制表（4点）
struct table2D IATDensityCorrectionTable;       ///< 进气温度密度修正表（9点）
struct table2D baroFuelTable;                   ///< 大气压力燃油修正表（8点）
struct table2D IATRetardTable;                  ///< 进气温度点火提前修正表（6点）
struct table2D idleTargetTable;                 ///< 怠速目标转速表（10点）
struct table2D idleAdvanceTable;                ///< 怠速点火提前角调整表（基于 RPM 差，6点）
struct table2D CLTAdvanceTable;                 ///< 冷却液温度点火提前角修正（6点）
struct table2D rotarySplitTable;                ///< 转子引擎点火分离角度表（8点）
struct table2D flexFuelTable;                   ///< 酒精混合比例燃油修正表（6点）
struct table2D flexAdvTable;                    ///< 酒精混合比例点火提前角修正（6点）
struct table2D flexBoostTable;                  ///< 酒精混合比例增压目标修正（6点）
struct table2D fuelTempTable;                   ///< 燃油温度修正表（6点）
struct table2D knockWindowStartTable;           ///< 爆震检测窗口起始角（2D）
struct table2D knockWindowDurationTable;        ///< 爆震检测窗口持续时间（2D）
struct table2D oilPressureProtectTable;         ///< 机油压力保护逻辑表（2D）
struct table2D wmiAdvTable;                     ///< 水甲醇喷射点火提前角修正（6点）
struct table2D coolantProtectTable;             ///< 冷却液温度保护表（2D）
struct table2D fanPWMTable;                     ///< 风扇 PWM 控制表（2D）
struct table2D rollingCutTable;                 ///< 滚动切断控制表（2D）
