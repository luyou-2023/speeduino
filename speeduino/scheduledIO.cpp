 ...
 */
 /**
 这些函数用于打开特定的喷油器（如 Injector1、Injector2 等）。
 injectorOutputControl 是一个全局变量，用于决定使用哪种喷油器控制方式。
 如果 injectorOutputControl 的值不等于 OUTPUT_CONTROL_MC33810，则调用 openInjectorX_DIRECT() 函数。
 否则，调用 openInjectorX_MC33810() 函数。
 openInjectorX_DIRECT 和 closeInjectorX_DIRECT：

 这些宏定义用于直接控制喷油器的开关，通过操作对应的引脚来实现。
 例如，openInjector1_DIRECT() 会将 inj1_pin_port 寄存器的相关位设置为高电平（1），从而打开喷油器1。
 openInjectorX_MC33810 和 closeInjectorX_MC33810：

 这些函数用于通过 MC33810 控制电路来控制喷油器。MC33810 是一种常用的喷油器控制芯片，它可以提供更复杂的控制功能，如电流调节和喷油器的延迟控制。
 这些函数的具体实现细节会依赖于 MC33810 芯片的操作手册和具体的应用需求。
 **/
