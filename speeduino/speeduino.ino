...
Speeduino 核心工作流程
初始化阶段
    加载各种标定表（燃油喷射、点火时刻、冷启动等），
    初始化硬件外设（定时器、中断、ADC等），
    预设一些初始状态和参数，准备发动机控制。
主循环（loop）
    周期性读取传感器数据（如节气门位置 TPS、冷却液温度、氧传感器等），
    进行状态判断（发动机启动、怠速、加速等），
    基于当前转速（RPM）和负载（TPS、MAP）查询标定表，计算喷油时间、点火提前角，
    调用对应函数执行喷油、点火调度，
    处理一些非实时但必须周期处理的任务，比如通讯、故障诊断等。
传感器中断（特别是曲轴和凸轮轴传感器）
    实时获取精确的发动机角度和转速，
    根据每个齿（tooth）触发的中断，快速更新发动机状态，
    在中断里执行或调整喷油和点火时机的精确定时（如adjustCrankAngle），
    确保喷油和点火动作与曲轴旋转同步，保障发动机运行稳定。
    额外补充
    中断驱动是关键，保证发动机点火喷油时机的高精度和低延迟响应。
    主循环多是做“管理和辅助”，而不是实时调度。
    传感器中断和主循环协同工作，保证发动机既精准又灵活地运行。

几乎所有现代汽油发动机控制单元（ECU）——无论是开源项目像Speeduino，还是商业品牌的ECU——在核心架构上都是类似的：
    传感器中断提供实时、高精度的发动机状态（曲轴位置、转速等），用于精准同步喷油和点火时机
    主循环处理非实时任务，管理状态，调整参数，执行辅助功能（如诊断、通讯等）
    初始化阶段加载各种参数、标定数据，设置硬件资源
    这种分层设计让系统既能保证极低延迟的控制响应，又能灵活处理多种复杂情况。

不同的ECU差别更多体现在：
    标定表和控制策略的复杂度和精细度
    传感器种类及信号处理能力
    硬件性能和扩展功能（比如自适应控制、故障自诊断、网络通讯等）
**/
